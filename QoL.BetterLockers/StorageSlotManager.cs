...
namespace QoL.BetterLockers
{
    partial class DropResourcesPatch
    {
        public static class StorageSlotManager
        {
            ...
            public class StorageSlotSettings
            {
                public SlotSettings[]? Box { get; set; }

                public SlotSettings[]? Locker { get; set; }
            }

            private const string CONFIG_FILE = DropResources.Plugin.PluginGUID + ".storageslots.json";
            ...
            {
                Instance!.LogDebug($"WATCH EVENT: {e.ChangeType} - {e.Name}");
                if (e.ChangeType is not WatcherChangeTypes.Changed and not WatcherChangeTypes.Created) return;
            ...
            private static void UpdateWireframes(List<RectangleWireframe[]> wireframes, Func<int, SlotSettings?> getSlotSettingsFunc)
            {
                for (var i = wireframes.Count - 1; i >= 0; i--)
                {
                    ...
                    {
                        Instance!.LogWarning($"Container {i} was garbage-collected");
            ...
            private static bool LoadSettings()
            {
                ...
                try
                {
                    ...
                    // XXX: unfortunately System.Text.Json doesn't have a
                    // non-async JsonSerializer.Deserialize(Stream) overload
                    var obj = JsonSerializer.Deserialize<StorageSlotSettings>(File.ReadAllText(jsonPath), jsonSettings);

                    BoxSlots = obj?.Box;
                    LockerSlots = obj?.Locker;
                    SettingsLoaded = true;
                    return true;
                }
                catch (Exception ex)
                {
                    Instance!.LogError($"Failed to deserialize wireframes config file: {ex}");
                    return false;
                }
            }
